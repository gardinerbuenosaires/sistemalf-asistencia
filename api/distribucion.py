...
def add_detalle(body: DetalleIn, user=Depends(require_permiso("distribucion", "editar"))):
    ...
    with db_session() as conn:
        ...
        # El chef ocupa un solo lugar por día. La fila CHEF es el comodín para cuando
        # no cubre ninguna plaza y sirve para que el contador cierre; en cuanto se le
        # da una plaza real, sale de CHEF en vez de figurar en los dos lados.
        if body.puesto_id is not None:
            destino = conn.execute(
                "SELECT es_chef FROM puestos WHERE id=?", (body.puesto_id,)
            ).fetchone()
            if destino and not destino["es_chef"]:
                conn.execute(
                    """DELETE FROM distribucion_detalle
                       WHERE distribucion_id=? AND empleado_id=? AND fecha=?
                         AND es_comida_personal=0 AND es_franco=0
                         AND puesto_id IN (SELECT id FROM puestos WHERE es_chef=1)""",
                    (body.distribucion_id, body.empleado_id, body.fecha)
                )

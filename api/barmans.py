...
def set_celda(body: CeldaIn, user=Depends(require_permiso("barmans", "editar"))):
    ...
    with db_session() as conn:
        ...
        conn.execute(
            "UPDATE barmans_semana SET modificado_por=?, modificado_en=datetime('now','localtime') WHERE id=?",
            (uid, semana_id)
        )
...
def _ejecutar_confirmacion_barmans(conn, semana, uid: int):
    ...
    conn.execute(
        "UPDATE barmans_semana SET estado='confirmado', modificado_por=?, modificado_en=datetime('now','localtime') WHERE id=?",
        (uid, semana_id)
    )
...
def desconfirmar_semana(semana_id: int, user=Depends(require_permiso("barmans", "confirmar"))):
    ...
    with db_session() as conn:
        ...
        conn.execute(
            "UPDATE barmans_semana SET estado='borrador', modificado_por=?, modificado_en=datetime('now','localtime') WHERE id=?",
            (uid, semana_id)
        )
    return {"ok": True}

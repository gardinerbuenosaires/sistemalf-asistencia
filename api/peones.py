...
def get_departamentos(_user=Depends(require_permiso("peones", "ver"))):
    with db_session() as conn:
        rows = conn.execute(
            """SELECT d.id, d.nombre, d.activo, d.sector_id,
                      d.usa_peones, d.escribe_planificacion,
...
def get_puestos(departamento_id: Optional[int] = None,
                _user=Depends(require_permiso("peones", "ver"))):
    with db_session() as conn:
        if departamento_id:
            rows = conn.execute(
...
@router.post("/puestos")
def create_puesto(body: PuestoIn, user=Depends(require_permiso("peones", "editar"))):
    with db_session() as conn:
...
def update_puesto(pid: int, body: PuestoIn, user=Depends(require_permiso("peones", "editar"))):
    with db_session() as conn:
        turno_val = body.turno if body.turno in ("TM", "TN", "CO") else None
        conn.execute(
...
def get_semana(departamento_id: int, turno: str, semana_inicio: str,
               user=Depends(require_permiso("peones", "ver"))):
    ...
    with db_session() as conn:
        ...
        dept_info = conn.execute(
            "SELECT usa_peones FROM departamentos WHERE id=?", (departamento_id,)
        ).fetchone()
        usa_dist = bool(dept_info and dept_info["usa_peones"])
...
def copiar_semana_anterior(body: CopiarSemanaIn, user=Depends(require_permiso("peones", "editar"))):
    ...
    with db_session() as conn:
        ...
        for fr in francos_ant:
            ...
            ya_en_puesto = conn.execute(
                """SELECT id FROM peones_detalle
                   WHERE distribucion_id=? AND empleado_id=? AND fecha=? AND es_franco=0""",
...
@router.post("/detalle")
def add_detalle(body: DetalleIn, user=Depends(require_permiso("peones", "editar"))):
    uid = int(user["sub"])
    with db_session() as conn:
        dist = conn.execute(
...
def confirmar_semana(dist_id: int, user=Depends(require_permiso("peones", "confirmar"))):
    uid = int(user["sub"])
    with db_session() as conn:
        dist = conn.execute(
        ...
        if choques:
            ...
            detalle = "; ".join(
                f"{nombres.get(e, e)} el {f}" for e, f in choques[:5]
            )
            if len(choques) > 5:
                detalle += f" (y {len(choques) - 5} más)"
            raise HTTPException(
                409,
                "Hay empleados con franco y puesto asignado el mismo día. "
                f"Corregí antes de confirmar: {detalle}"
            )
...
def get_mis_accesos(user=Depends(require_permiso("peones", "ver"))):
    ...
    with db_session() as conn:
        ...
        depts = conn.execute(
            f"""SELECT d.id, d.nombre, d.sector_id, s.nombre AS sector_nombre
                FROM departamentos d
                LEFT JOIN sectores_legajo s ON s.id = d.sector_id
                WHERE d.id IN ({placeholders}) AND d.activo=1 AND d.usa_peones=1
                ORDER BY s.nombre, d.nombre""",
            dept_ids
        ).fetchall()
...
def get_personal_dept(departamento_id: int, user=Depends(require_permiso("peones", "editar"))):
...
# ── usa_peones: activar / desactivar por departamento ───────────────────

@router.get("/departamentos/{dept_id}/usa-distribucion-preview")
def preview_usa_peones(dept_id: int, _user=Depends(require_permiso("peones", "editar"))):
    """Devuelve info básica del departamento para confirmar la activación."""
    with db_session() as conn:
        dept = conn.execute(
            "SELECT id, nombre, usa_peones FROM departamentos WHERE id=?", (dept_id,)
        ).fetchone()
        if not dept:
    ...
    return {
        ...
        "nombre": dept["nombre"],
        "usa_peones": bool(dept["usa_peones"]),
...
def set_usa_peones(dept_id: int, body: UsaPeonesIn,
    ...
    with db_session() as conn:
        ...
        if not dept:
            raise HTTPException(404, "Departamento no encontrado")
        conn.execute(
            "UPDATE departamentos SET usa_peones=? WHERE id=?",
